"""
Sentiment Classification with Naive Bayes - Utils helper functions File
Vikram Singh
"""
...
def get_tokens(data):
    ...
    stopwords = set(readFile('data/english.stop'))
    result = [nltk.word_tokenize(datapoint.lower()) for datapoint in data['content'].tolist()]
    final_result = []
    for line in result:
        final_result.append([word for word in line if not word in stopwords and word.strip()!=''])
    return final_result
...
def get_data(data_fn):
    ...
    # Load the data
    df = pd.read_csv(data_fn, header='infer')
...
def vectorize_data(train, dev, test, batch_size, embedding_dim):
    ...
    print(label_enc.inverse_transform(np.arange(len(LABEL_NAMES))))
    print("\n\n")
